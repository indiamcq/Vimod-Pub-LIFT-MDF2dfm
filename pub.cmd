...
if "%testoutfile%" == "" echo %curcommand%>>%projectlog%
if "%testoutfile%" neq "" call :before
if "%commandpath%" neq "" (
  set startdir=%cd%
  set drive=%commandpath:~0,2%
  %drive%
  cd "%commandpath%"
  echo Path changed to: %cd%
) 
call %curcommand%
if "%commandpath%" neq "" (
  ...
  cd "%startdir%"
  set dive=
  echo Path restored to: %cd%
)
if "%testoutfile%" neq "" call :after
if defined masterdebug call :funcdebugend
goto :eof
...
:echo
:: Description: turns echo on or off for debugging
:: Required parameters:
:: argument
set argument=%~1
if "%argument%" neq "" set echoswitch=valid
if "%argument%" == "on" set echoswitch=switch
if "%argument%" == "off" set echoswitch=switch
if "%argument%" == "." set echoswitch=blank
if "%argument%" == " " set echoswitch=blank
if "%echoswitch%" neq "" (
  if "%echoswitch%" == "blank" @echo.
  if "%echoswitch%" == "switch" @echo %argument%
  if "%echoswitch%" == "valid" echo %argument%
) else (
    echo Invalid argument for echo function: "%argument%"  
)
set echoswitch=
goto :eof
...
call :after "command with %curcommand% output to %outfile% complete"
if defined masterdebug call :funcdebugend
goto :eof
