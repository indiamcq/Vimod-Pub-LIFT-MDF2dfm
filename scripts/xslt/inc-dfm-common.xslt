...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
      <xsl:variable name="nomarkupgroup_list" select="
'CommonGroups
psGroup
...
"/>
	...
	It is okay to have redundant groups, but all groups sould be included and any thing else that has valid content -->
      <xsl:variable name="nomarkupgroup" select="tokenize($nomarkupgroup_list,'\s+')"/>
...
<!-- <xsl:if test="$size = 'wl'">
<xsl:text>{{</xsl:text>
<xsl:value-of select="following-sibling::ge[1]"/>
<xsl:value-of select="following-sibling::gn[1]"/>
<xsl:text>}}</xsl:text>
</xsl:if> -->
            <xsl:if test="$size != 'wl'">
